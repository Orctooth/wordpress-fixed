...
function ovation_blog_notice(){
    global $pagenow;
    if ( is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated'] ) ) {
   		wp_safe_redirect( admin_url("themes.php?page=ovation-blog-guide-page") );
   	}
}
add_action('after_setup_theme', 'ovation_blog_notice');

function ovation_blog_add_new_page() {
    $edit_page = admin_url().'post-new.php?post_type=page';
    echo json_encode(['page_id'=>'','edit_page_url'=> $edit_page ]);

    exit;
}
add_action( 'wp_ajax_ovation_blog_add_new_page','ovation_blog_add_new_page' );